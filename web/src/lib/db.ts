...
export interface ResultCardFrozen {
  ...
  /** The register behind attendance_pct, added by 0105. A percentage on a card
   *  that goes home is unarguable only if the days behind it are printed too:
   *  the school grants leave and the same card then reads as though the child
   *  did not turn up. Optional like every field below, because a reprint of an
   *  older card has a frozen payload that never had it. */
  attendance?: {
    present: number; absent: number; leave: number; late: number
    half_day: number; marked_days: number; present_pct: number | null
  } | null
...
export interface PortalAttendance {
  ...
  marked: number
  percent: number | null
  /** Every status the register can hold. These four were already returned by
   *  fn_portal_child_attendance and simply were not declared here, which is how
   *  `leave` stayed invisible on the parent's own screen: nothing in the app
   *  could see a field the type did not admit existed. `leave` was added by
   *  0105; the other three by 0097. Optional, because a parent whose school has
   *  not applied bundle 13 yet still gets an object without `leave`. */
  absent?: number
  late?: number
  half_day?: number
  leave?: number
